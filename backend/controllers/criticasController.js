...
async function recalcularMediaEAtualizarLivro(id_livro) {
  const rows = await query(
    'SELECT COUNT(*) AS qtd, AVG(nota) AS media FROM criticas WHERE id_livro = ?',
    [id_livro]
  );
  const qtd = rows[0].qtd || 0;
  const media =
    rows[0].media === null ? null : Number(parseFloat(rows[0].media).toFixed(1));

  await query(
    'UPDATE livros SET media_avaliacao = ?, qtd_avaliacoes = ? WHERE id_livro = ?',
    [media, qtd, id_livro]
  );
  return { media, qtd };
}
...
module.exports = {
    ...
    try {
      ...
      if (usuario.cargo !== 'critico')
        return res.status(403).json({
          success: false,
          message: 'Apenas críticos podem criar críticas.',
        });
      ...
      if (!id_livro || !texto || nota === undefined) {
        return res.status(400).json({
          success: false,
          message: 'Campos obrigatórios: id_livro, texto, nota.',
        });
      }
      ...
      if (texto.length > 100)
        return res.status(400).json({
          success: false,
          message: 'Texto deve ter no máximo 100 caracteres.',
        });
      ...
      if (notaNum < 0 || notaNum > 100)
        return res.status(400).json({
          success: false,
          message: 'Nota deve ser entre 0 e 100.',
        });
      ...
      if (existe.length > 0) {
        return res.status(409).json({
          success: false,
          message: 'Você já possui uma crítica para este livro.',
        });
      }
      ...
      return res.status(201).json({
        ...
        qtd_avaliacoes: stats.qtd,
            ...
            console.error("Erro ao criar crítica:", err);
  ...
  async atualizarCritica(req, res) {
    try {
      ...
      const rows = await query(
        'SELECT id_usuario, id_livro FROM criticas WHERE id_critica = ?',
        [id_critica]
      );
      if (rows.length === 0)
      ...
      if (texto !== undefined) {
        if (texto.length > 100)
          return res
            .status(400)
            .json({ erro: 'Texto deve ter até 100 caracteres.' });
        updates.push('texto = ?');
        params.push(texto);
      }
      ...
      if (nota !== undefined) {
        ...
        if (notaNum < 0 || notaNum > 100)
          return res
            .status(400)
            .json({ erro: 'Nota deve ser entre 0 e 100.' });
        updates.push('nota = ?');
        params.push(notaNum);
      }
      ...
      await query(
        `UPDATE criticas SET ${updates.join(', ')} WHERE id_critica = ?`,
        params
      );
    ...
    try {
      ...
      return res.status(200).json({
        ...
        media:
          stats[0].media === null
            ? null
            : Number(stats[0].media).toFixed(1),
        qtd: stats[0].qtd,
      });
    } catch {
      return res.status(500).json({ erro: 'Erro interno.' });
